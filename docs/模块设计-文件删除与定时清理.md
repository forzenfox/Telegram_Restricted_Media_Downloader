...
| 服务重启 | `start()` 重新加载任务；错过窗口（<1 周期）启动后立即补一轮；paused 状态保持。`cleanup_files` 属常驻 running 型任务，经 `TaskManager.RESIDENT_RUNNING_TASK_TYPES` 常量豁免启动时的 stale-running 失败标记，重启后保持 running 由 `CleanupScheduler` 续跑 |
